...
$DATA_FILE  = __DIR__.'/data/atlit.json';
$UPLOAD_DIR = __DIR__.'/uploads/atlit';
...
function save_upload($field,$prefix){
  ...
  return 'uploads/atlit/'.basename($dest);
}
function need_admin(){
...
switch ($action){
  ...
  case 'create':
    ...
    $rec = [
      ...
      'medali'=>$_POST['medali'] ?? [], // array medalinya
      'jumlah'=>$_POST['jumlah'] ?? [],
      'tahun'=>$_POST['tahun'] ?? [],
      'event'=>$_POST['event'] ?? [],
      'sejarah_singkat'=>trim($_POST['sejarah_singkat']??''),
      'foto'=>$foto
    ];
    if ($rec['nama']==='') fail('Nama atlit wajib',400);
    $data[] = $rec;
  ...
  case 'update':
    ...
    $i  = idx_of($data,$id);
    if ($i<0) fail('Not found',404);
    foreach(['nama','jk','tgl_lahir','tempat_lahir','sejarah_singkat'] as $f){
      if (isset($_POST[$f])) $data[$i][$f] = trim($_POST[$f]);
    }
    $data[$i]['medali'] = $_POST['medali'] ?? [];
    $data[$i]['jumlah'] = $_POST['jumlah'] ?? [];
    $data[$i]['tahun']  = $_POST['tahun'] ?? [];
    $data[$i]['event']  = $_POST['event'] ?? [];
    if ($new = save_upload('foto','at_'.$id)) $data[$i]['foto'] = $new;
  ...
  case 'delete':
    ...
    if (!empty($data[$i]['foto'])){
      ...
      if (strpos(realpath($p), realpath(__DIR__.'/uploads/atlit'))===0 && file_exists($p)) @unlink($p);
    }
