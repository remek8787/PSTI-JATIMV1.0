...
const SCHEMA={
  ...
  pengurus:[['nama','text',1],['jabatan','text'],['kategori','text'],['kat_order','number'],['urut','number'],['alamat','text'],['telepon','text'],['pendidikan','text'],['sertifikat','text'],['biografi','textarea'],['foto','file']],
  klub:[['nama','text',1],['kabupaten','text'],['pemilik','text'],['telepon','text'],['alamat','textarea'],['instagram','text'],['foto','file']]
};
...
function makeInput(name,type,val=''){if(type==='textarea')return `<label>${name}<textarea class='input' name='${name}'>${esc(Array.isArray(val)?val.join('\n'):val??'')}</textarea></label>`; if(type==='checkbox')return `<label><input type='checkbox' name='${name}' value='1' ${Number(val)?'checked':''}> ${name}</label>`; return `<label>${name}<input class='input' type='${type==='file'?'file':type||'text'}' name='${name}' ${type==='file'?'accept="image/*"':''} value='${type==='file'?'':esc(val??'')}'></label>`;}
function openForm(mod,data={}){const m=$('#modal');const body=$('#modalBody');$('#formTitle').textContent=(data.id?'Edit ':'Tambah ')+mod;body.innerHTML=`<form id='f' class='form-grid'><input type='hidden' name='id' value='${esc(data.id||'')}'>${SCHEMA[mod].map(([n,t])=>`<div class='${t==='textarea'?'full':''}'>${makeInput(n,t,data[n])}</div>`).join('')}<div class='full'><button class='btn' type='submit'>Simpan</button></div></form>`;m.showModal();$('#f').onsubmit=async(e)=>{e.preventDefault();const fd=new FormData(e.target);fd.append('action',data.id?'update':'create');if(mod==='berita'&&!fd.get('published_at'))fd.set('published_at',new Date().toISOString().slice(0,19));for(const [n,t] of SCHEMA[mod]){if(t==='textarea'&&['medali','jumlah','tahun','event','sertifikat','tahun_sertifikat','pengalaman','tahun_pengalaman','prestasi','tahun_prestasi'].includes(n)){fd.set(n,(fd.get(n)||'').toString());}}
const r=await fetch(API[mod],{method:'POST',body:fd});const j=await r.json();if(!j.ok)return toast(j.error||'Gagal simpan',0);m.close();toast('Berhasil disimpan');loadPanel(mod);};}

function renderSimpleRows(mod,items){const w=$('#table-'+mod); if(!w)return; const row=(mod==='agenda')?['periode','kegiatan']:['klub','main','menang','poin'];
  w.innerHTML=`<div id='rows'>${items.map(it=>`<div class='form-grid'>${row.map(k=>`<input class='input' data-k='${k}' value='${esc(it[k]??'')}'>`).join('')}</div>`).join('')}</div><button id='addRow' class='btn secondary' style='margin-top:.6rem'>Tambah Baris</button><button id='saveRows' class='btn' style='margin-top:.6rem'>Simpan</button>`;
  $('#addRow',w).onclick=()=>$('#rows',w).insertAdjacentHTML('beforeend',`<div class='form-grid'>${row.map(k=>`<input class='input' data-k='${k}' placeholder='${k}'>`).join('')}</div>`);
  $('#saveRows',w).onclick=async()=>{const rows=$$('#rows > div',w).map(d=>Object.fromEntries(row.map(k=>[k,$(`[data-k="${k}"]`,d).value]))); const r=await fetch(API[mod]+'?action=save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:rows})}); const j=await r.json(); if(!j.ok)return toast(j.error||'Gagal simpan',0); toast('Tersimpan');};
}
...
async function loadPanel(mod){const j=await list(mod);if(!j.ok)return toast(j.error||'Gagal load',0); if(['agenda','klasemen'].includes(mod))return renderSimpleRows(mod,j.items||[]); if(mod==='sponsor') return renderSponsor(j.items||[]); renderTable(mod,j.items||[]);}
